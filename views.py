...
class NaveLista(ListView):
 ...
 def get_queryset(self):
    ...
    qs = (Nave.objects.select_related("armador").annotate(cert_vigentes = Count("certificados", filter = Q(certificados__fecha_vencimiento__gte=self.request.today))))
    if q:
...
class CertificadoCrear(CreateView):
 ...
 def get_success_url(self):
    return reverse_lazy("naves:nave_detail", kwargs={"pk":
self.kwargs["pk"]})
